...
export function HomePage() {
  ...
  const popular = useQuery({
    ...
    queryFn: () => api.searchManga({ limit: 18 }),
  });
  const latest = useQuery({
    queryKey: ['manga', 'latest'],
    queryFn: () => api.searchManga({ q: 'one', limit: 12 }),
  });
            ...
            {library.isLoading ? (
              <div className='grid gap-3 sm:grid-cols-2'>
                {Array.from({ length: 4 }).map((_, index) => (
                  <div
                    key={index}
                    className='h-24 animate-pulse rounded-lg border border-[var(--line)] bg-[var(--surface-strong)]'
                  />
                ))}
              </div>
            ) : recentlyRead.length ? (
      ...
      >
        {item.manga?.coverUrl ? (
          <img
            ...
            src={assetUrl(item.manga.coverUrl)}
            alt={title}
          />
        ) : (
          <BookOpen className='m-auto h-full text-[var(--muted)]' />
        )}
      </Link>
      <div className='min-w-0'>
        <h2 className='line-clamp-2 text-2xl font-black'>{title}</h2>
        <p className='mt-2 text-sm text-[var(--muted)]'>
          {progress ? `Page ${progress.pageIndex + 1}` : 'Ready to continue'} ·{' '}
        ...
        <div className='mt-4 flex flex-wrap gap-2'>
          {chapterId ? (
            <Link
              ...
              to={`/read/${chapterId}?mangaId=${item.mangaId}`}
            >
              <Play size={18} />
              Continue
            </Link>
...
function RecentCard({ item }: { item: LibraryItem }) {
    ...
    <article className='grid grid-cols-[48px_1fr] gap-3 rounded-lg border border-[var(--line)] bg-[var(--accent-panel)] p-2.5'>
      ...
      <div className='min-w-0'>
        ...
        <p className='mt-1 text-xs text-[var(--muted)]'>
          {progress ? `Page ${progress.pageIndex + 1}` : item.status} ·{' '}
          {formatActivityDate(item)}
        </p>
      </div>
    </article>
  ...
  return chapterId ? (
    <Link to={`/read/${chapterId}?mangaId=${item.mangaId}`}>{content}</Link>
  ) : (
    <Link to={`/manga/${item.mangaId}`}>{content}</Link>
  );
}
  ...
  return (
    <div className='manga-grid'>
      {Array.from({ length: 12 }).map((_, index) => (
        <div
          key={index}
          className='h-72 animate-pulse rounded-lg border border-[var(--line)] bg-[var(--surface)]'
        />
      ))}
    </div>
  );
