...
export function SearchPage() {
  ...
  const preset = location.pathname.includes('/discover/latest')
    ? 'latest'
    : location.pathname.includes('/discover/popular')
      ? 'popular'
      : 'search';
    ...
    <div className='space-y-6'>
      <section className='surface rounded-lg p-5'>
        <div className='mb-5 flex flex-wrap items-start justify-between gap-4'>
          <div>
            ...
            <h1 className='text-2xl font-black'>
              {getDiscoveryTitle(preset, routeGenre)}
            </h1>
            <p className='mt-2 max-w-2xl text-sm text-[var(--muted)]'>
              Search MangaDex, tune the shelf filters, and keep the results
              focused on readable VI/EN chapters.
            </p>
          </div>
          <Link
            className='btn min-h-10 text-sm'
            to={preset === 'popular' ? '/discover/latest' : '/discover/popular'}
          >
        ...
        <div className='grid gap-3 lg:grid-cols-[1fr_220px_120px] lg:items-end'>
          ...
          <label>
            ...
            <select
              ...
              value={sort}
              onChange={(event) =>
                setSort(event.target.value as MangaDiscoverySort)
              }
        ...
        <div className='mt-5 grid gap-4 xl:grid-cols-[1fr_1fr]'>
          <FilterPanel
            title='Include tags'
            subtitle='Use these as manga shelf labels.'
          >
            {genres.isLoading ? (
              <ChipSkeleton />
            ) : (
          ...
          <FilterPanel
            title='Exclude tags'
            subtitle='Hide tags that do not fit the mood.'
          >
            {genres.isLoading ? (
              <ChipSkeleton />
            ) : (
        ...
        <div className='mt-4 grid gap-4 lg:grid-cols-[1fr_1fr_180px]'>
          <FilterPanel title='Content rating'>
            <ToggleGroup
              ...
              onToggle={(value) =>
                toggleRequiredListValue(setContentRating, value)
              }
            />
          </FilterPanel>
          <FilterPanel title='Status'>
            <ToggleGroup
              ...
              values={status}
              onToggle={(value) => toggleListValue(setStatus, value)}
            />
          </FilterPanel>
        ...
        <div className='mt-4 flex flex-wrap gap-2 text-xs font-bold text-[var(--muted)]'>
          ...
          {status.map((item) => (
            <button
              key={item}
              className='chapter-legend'
              onClick={() => toggleListValue(setStatus, item)}
              type='button'
            >
              {item}
            </button>
          ...
          {validYear ? (
            <span className='chapter-legend'>Year: {validYear}</span>
          ) : null}
        </div>
      </section>

      {result.isError ? (
        <div className='surface rounded-lg p-6 text-[var(--danger)]'>
          {result.error.message}
        </div>
      ) : result.isLoading ? (
        <div className='surface rounded-lg p-6 text-[var(--muted)]'>
          Searching...
        </div>
      ) : (
        <div className='space-y-5'>
          <SourceBanner
            source={source}
            hasTagFilters={includedTags.length > 0 || excludedTags.length > 0}
          />
          {manga.length ? (
  ...
  return (
    <div className='flex flex-wrap gap-2'>
      {options.map((option) => (
        <button
          key={option.value}
          className={`genre-chip ${values.includes(option.value) ? 'genre-chip-active' : ''}`}
  ...
  return (
    <div className='flex flex-wrap gap-2'>
      {Array.from({ length: 10 }).map((_, index) => (
        <span
          key={index}
          className='h-9 w-24 animate-pulse rounded-full border border-[var(--line)] bg-[var(--surface-strong)]'
        />
      ))}
    </div>
  );
...
function SourceBanner({
  ...
  return (
    <div className='surface rounded-lg p-4 text-sm text-[var(--accent)]'>
      {hasTagFilters
        ? 'Showing cached manga for tag filters. Sync more MangaDex data if the shelf feels sparse.'
        : 'Showing cached data while MangaDex is unavailable.'}
    </div>
  );
}
...
function EmptyDiscoveryState({ hasFilters }: { hasFilters: boolean }) {
  return (
    <div className='surface rounded-lg p-8 text-center text-[var(--muted)]'>
      {hasFilters
        ? 'No manga matches this discovery mix. Broaden filters, remove excluded tags, or clear the year.'
        : 'No manga found. Try another title or sync more MangaDex data.'}
    </div>
  );
}
...
function toggleRequiredListValue<T extends string>(
  setValues: React.Dispatch<React.SetStateAction<T[]>>,
  value: T,
) {
  setValues((current) => {
