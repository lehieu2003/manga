...
import { refreshSessionRepository, userRepository } from "../../domain/repositories/index.js";
...
import type { changePasswordSchema, loginSchema, refreshSchema, registerSchema, updateProfileSchema } from "../validators/auth.validator.js";
...
export async function loginUser(app: FastifyInstance, input: LoginInput) {
  ...
  const tokens = await issueTokenPair(app, user);
  await domainEvents.publish({ type: "auth.user_logged_in", userId: user.id });
  return { user: publicUser(user), ...tokens };
}

export async function refreshAuthToken(app: FastifyInstance, input: RefreshInput) {
  const payload = await rotateRefreshToken(app, input.refreshToken);
  await domainEvents.publish({ type: "auth.refresh_token_rotated", userId: payload.user.id });
...
export async function changeCurrentUserPassword(app: FastifyInstance, userId: string, input: ChangePasswordInput) {
  const user = await userRepository.findByIdOrThrow(userId);
  if (!(await verifyPassword(input.currentPassword, user.passwordHash))) {
    throw new HttpError(401, "Current password is incorrect", "INVALID_CURRENT_PASSWORD");
  }

  const updatedUser = await userRepository.updatePassword(user.id, await hashPassword(input.newPassword));
  await revokeUserRefreshSessions(updatedUser.id);
  const tokens = await issueTokenPair(app, updatedUser);
  await domainEvents.publish({ type: "auth.password_changed", userId: updatedUser.id });
